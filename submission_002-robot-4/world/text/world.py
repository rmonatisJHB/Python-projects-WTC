...
def do_forward(robot_name, steps):
    ...
    if update_position(steps,robot_name):
...
def draw_obstacles():
    """
    lists all the obstacles present
    """
    list_of_obs = obstacles.get_obstacles()
    if len(list_of_obs) > 0:
        print("There are some obstacles:")
        for i in list_of_obs:
            print(f"- At position {i[0][0]},{i[0][1]} (to {i[0][0] + 4},{i[0][1] + 4})")
